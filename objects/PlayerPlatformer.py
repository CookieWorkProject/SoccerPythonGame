...
import numpy
import copy
class PlayerPlatformer(BasicObject):
    ...
    def stepMovement(self):
        ...
        #downplatform
        if (self.controller.confirm and 
            self.controller.down and
            self.isGrounded() and 
            self.meetingObjectType(self.getRectOffset(y=1),BlockDown) and
            not self.meetingObjectType(self.getRectOffset(y=1),Solid)):
            self.rect.top+=2
        #jump
        elif (self.controller.confirm and 
            self.isGrounded()):
        ...
        self.moveSelf(xMove,yMove)
    def isGrounded(self):
        return self.meetingList(self.getRectOffset(y = 1),self.getAllValidSolids(self.xVelo,self.yVelo))
        
    def moveSelf(self,xMove,yMove):
        solidList = self.getAllValidSolids(xMove,yMove)
        print(str(len(solidList)))
        #push in X
    ...
    def getAllValidSolids(self,xMove=0,yMove=0):
        ...
        for a in self.gameObjects:
            if type(a) == Solid:
                objects.append(a)
            if type(a) == BlockLeft and xMove>=0 and self.getRightX()<a.rect.left+2:
                objects.append(a)
            if type(a) == BlockRight and xMove<=0 and self.rect.left>a.getRightX()-2:
                
                objects.append(a)
            if type(a) == BlockUp and yMove<=0 and self.rect.top>a.getBottomY()-2:
                objects.append(a)
            if type(a) == BlockDown and yMove>=0 and self.getBottomY()<a.rect.top+2:
                objects.append(a)    
    ...
    def collision(self,gameObjects):
        for a in gameObjects:
            ...
            if type(a) == Coin:
                ...
                self.enemyCollision(gameObjects,a)
            elif type(a) == PickUp:
                if self.controller.confirm:
                    self.pickUpCollision(gameObjects,a)
